...
export function Footer() {
  ...
  const sectionLink = (hash: string, label: string) => {
    if (isHome) {
      return (
        <a href={hash} className="text-white/70 hover:text-white transition-colors">
          {label}
        </a>
      )
    }
    return (
      <Link to={`/${hash}`} className="text-white/70 hover:text-white transition-colors">
        {label}
      </Link>
    )
  }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 mb-10">
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm">
                <li><Link to={ROUTE_PATHS.HOME} className="text-white/70 hover:text-white transition-colors">{t('home')}</Link></li>
                <li>{sectionLink('#passport-section', t('footerWhoWeAre'))}</li>
                <li>{sectionLink('#destinazioni-section', t('destinations'))}</li>
                ...
                <li><Link to={ROUTE_PATHS.TERMS} className="text-white/70 hover:text-white transition-colors">Termini e Condizioni</Link></li>
              </ul>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex gap-4">
              <a href="https://instagram.com/maiaviaggi" target="_blank" rel="noopener noreferrer" aria-label="Instagram MAIA Viaggi" className="text-white/60 hover:text-white transition-colors"><FaInstagram className="w-5 h-5" /></a>
              <a href="https://tiktok.com/@maiaviaggi" target="_blank" rel="noopener noreferrer" aria-label="TikTok MAIA Viaggi" className="text-white/60 hover:text-white transition-colors"><FaTiktok className="w-5 h-5" /></a>
              <a href="https://facebook.com/maiaviaggi" target="_blank" rel="noopener noreferrer" aria-label="Facebook MAIA Viaggi" className="text-white/60 hover:text-white transition-colors"><FaFacebook className="w-5 h-5" /></a>
            </div>
          </div>

          {/* Payments (coming soon) + Legal */}
          <div>
            <h3 className="font-heading text-lg italic text-[#DCB38C] mb-3 font-normal">{t('footerPayments')}</h3>
            <p className="text-sm text-white/50 mb-2">
              Pagamenti online non ancora disponibili.
            </p>
            <p className="text-xs text-white/30 italic">
              Visa, Mastercard, PayPal, Klarna, Scalapay e altri metodi saranno disponibili a breve.
            </p>
